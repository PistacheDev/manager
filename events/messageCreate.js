...
const { generateNumber } = require("../functions/generator");
    ...
    async run(client, db, message)
    {
        ...
        db.query("SELECT * FROM config WHERE guild = ?", [guild.id], async (err, config) =>
        {
            ...
            db.query("SELECT * FROM xp WHERE guild = ? AND user = ?", [guild.id, author.id], async (err, data) =>
            {
                ...
                if (maxLevel > level && 500 + (level * 10) <= xp)
                {
                ...
                else db.query("UPDATE xp SET xp = ? WHERE guild = ? AND user = ?", [xp, guild.id, author.id]);
            });
        });
    }
};