    ...
    async run(req, res)
    {
        try
        {
            ...
            db.query("SELECT * FROM config WHERE guild = ?", [guild.id], async (err, config) =>
            {
                ...
                {
                    if (role.name == "@everyone" || role.permissions.has(Perms.Administrator) || role.permissions.has(Perms.ManageGuild) || member.roles.highest.comparePositionTo(role) <= 0) return;
