    ...
    async run(client, db, interaction)
    {
        try
        {
            ...
            db.query("SELECT * FROM config WHERE guild = ?", [guild.id], async (err, config) =>
            {
                ...
                .addFields([{ name: ":gear:・XP system:", value: `➜ :green_circle: Gives XP points to the members per **each message sent**, **between 1** and **${maxXP} XP points**. Each time the members pass the **goal to level up**, their level **increase by 1**. The maximum level is **the level ${maxLevel}**. The members **${alert == "yes" ? "are" : "aren't"} notified** when they level up.` }])
