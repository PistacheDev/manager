    ...
    async run(client, db, interaction)
    {
        try
        {
            ...
            db.query('SELECT * FROM config WHERE guild = ?', [guild.id], async (err, data) =>
            {
                ...
                db.query('UPDATE config SET messagesLogs = ? WHERE guild = ?', [newChannel ? newChannel : 0, guild.id], async () =>
                {
