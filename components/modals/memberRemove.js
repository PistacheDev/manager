    ...
    async run(client, db, interaction)
    {
        try
        {
            ...
            db.query('SELECT * FROM config WHERE guild = ?', [interaction.guild.id], async (err, data) =>
            {
                ...
                db.query('UPDATE config SET memberRemove = ? WHERE guild = ?', [!newChannel ? null : newChannel, interaction.guild.id], async () =>
                {
                    // Generate the fields.
                    ...
                    embed.addFields([{ name: ':airplane_departure:・Departure Messages:', value: `>>> **Status**: ${!newChannel ? ':x: Inactive' : `:white_check_mark: Active.\n**Configured Channel**: <#${newChannel}>`}.\n**Function**: **Sends a message** in the **configured channel** when a user **leaves the server**.` }])
