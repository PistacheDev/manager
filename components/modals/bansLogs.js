    ...
    async run(client, db, interaction)
    {
        try
        {
            ...
            db.query('SELECT * FROM config WHERE guild = ?', [interaction.guild.id], async (err, data) =>
            {
                ...
                db.query('UPDATE config SET bansLogs = ? WHERE guild = ?', [!newChannel ? null : newChannel, interaction.guild.id], async () =>
                {
                    // Generate the fields.
